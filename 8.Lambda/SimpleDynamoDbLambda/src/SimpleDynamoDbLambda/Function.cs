...
public class Function
{
    public void FunctionHandler(DynamoDBEvent dynamoEvent, ILambdaContext context)
    {
        ...
        foreach (var record in dynamoEvent.Records)
        {
            ...
            if (!record.Dynamodb.OldImage.Any())
            {
                var oldDocument = Document.FromAttributeMap(record.Dynamodb.OldImage);
            
                context.Logger.LogInformation($"Old doc: {oldDocument.ToJsonPretty()}");
            }

            var newDocument = Document.FromAttributeMap(record.Dynamodb.NewImage);
            
            context.Logger.LogInformation($"New doc: {newDocument.ToJsonPretty()}");
